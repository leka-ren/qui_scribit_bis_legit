...
func (s ParcelStore) Add(p Parcel) (int, error) {
	result, err := s.db.Exec("INSERT INTO parcel (client, address, status, created_at) VALUES (:client, :address, :status, :created_at)", sql.Named("client", p.Client), sql.Named("address", p.Address), sql.Named("status", p.Status), sql.Named("created_at", p.CreatedAt))
	if err != nil {
		return 0, err
	}
...
func (s ParcelStore) GetByClient(client int) ([]Parcel, error) {
	queryRes, err := s.db.Query("SELECT number, client, address, status, created_at FROM parcel WHERE client = :client", sql.Named("client", client))
	if err != nil {
		return nil, err
	}
	...
	for queryRes.Next() {
		var p Parcel
		err := queryRes.Scan(&p.Number, &p.Client, &p.Address, &p.Status, &p.CreatedAt)
		if err != nil {
			return nil, err
		}
		res = append(res, p)
